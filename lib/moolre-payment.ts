...
export async function initMoolrePayment(args: {
  ...
  // POS08 = Moolre has no Web POS terminal for this account (settlement info incomplete).
  // Hide the raw SQL error from customers and surface a friendly message.
  const raw = String(result.message || '');
  if (result.code === 'POS08' || /terminal_id/i.test(raw)) {
    console.error('[Moolre] Web POS terminal not configured (settlement info incomplete):', raw);
    return {
      success: false,
      message: 'Mobile Money payment is temporarily unavailable. Please try again shortly or contact support.',
    };
  }
